...
import Image from 'next/image'; // <-- Importujeme optimalizovaný Image komponent
...
export default function PathwaysPage() {
    ...
    useEffect(() => {
        const fetchPathways = async () => {
            ...
            if (data && data.length > 0) {
                ...
                setActivePathway(pathwayFromUrl || data[0]);
            }
            setIsLoading(false);
        };
        fetchPathways();
    ...
    // Dynamicky meníme titulok stránky pre lepšie SEO
    useEffect(() => {
        if (activePathway) {
